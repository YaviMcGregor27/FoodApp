-- FoodApp: migración inicial para Supabase (PostgreSQL).
-- Estado: fase F0. Probada en integración continua sobre PostgreSQL 16 con una
-- réplica mínima del esquema `auth` de Supabase (supabase/pruebas/00_stub_supabase.sql).
--
-- Convenciones:
...
--   * Toda tabla con datos personales tiene user_id y política RLS para el rol
--     `authenticated`, basada en auth.uid() (el usuario del token de sesión).
--   * El rol `anon` no tiene acceso a ninguna tabla.
--   * Las funciones internas viven en el esquema `app`, que la API no expone.
...
CREATE TABLE app_user (
  ...
  email             text UNIQUE,                  -- puede faltar en algunos proveedores federados
  email_verified_at timestamptz,
  birth_date        date CHECK (birth_date >= date '1900-01-01'),
  registration_completed_at timestamptz,        -- fecha de nacimiento y términos aceptados
...
CREATE INDEX food_name_trgm ON food USING gin (name extensions.gin_trgm_ops);
...
CREATE INDEX user_product_name_trgm ON user_product USING gin (name extensions.gin_trgm_ops);
...
REVOKE UPDATE, DELETE ON inventory_movement FROM authenticated;

-- app_user: cada usuario ve y edita solo su fila.
ALTER TABLE app_user ENABLE ROW LEVEL SECURITY;
CREATE POLICY app_user_self ON app_user FOR ALL TO authenticated
  USING (id = (SELECT auth.uid())) WITH CHECK (id = (SELECT auth.uid()));
-- El usuario solo puede cambiar sus preferencias; el resto lo gestionan funciones del sistema.
GRANT SELECT ON app_user TO authenticated;
GRANT UPDATE (locale, time_zone, unit_system) ON app_user TO authenticated;
...
GRANT SELECT ON food, food_nutrient, recipe_ingredient TO authenticated;
ALTER TABLE recipe ENABLE ROW LEVEL SECURITY;
CREATE POLICY recipe_read ON recipe FOR SELECT TO authenticated
  USING (owner_user_id IS NULL OR owner_user_id = (SELECT auth.uid()));
CREATE POLICY recipe_write ON recipe FOR ALL TO authenticated
...
CREATE POLICY recipe_ingredient_read ON recipe_ingredient FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM recipe r WHERE r.id = recipe_id));
CREATE POLICY recipe_ingredient_write ON recipe_ingredient FOR ALL TO authenticated
  USING (EXISTS (SELECT 1 FROM recipe r WHERE r.id = recipe_id AND r.owner_user_id = (SELECT auth.uid())))
  WITH CHECK (EXISTS (SELECT 1 FROM recipe r WHERE r.id = recipe_id AND r.owner_user_id = (SELECT auth.uid())));
GRANT INSERT, UPDATE, DELETE ON recipe_ingredient TO authenticated;

-- ---------------------------------------------------------------------------
-- Purga de cuenta (RF-18). La ejecuta el proceso interno de eliminación (rol
-- service_role), nunca el usuario final. Borra en orden de dependencias dentro de una única
...
GRANT EXECUTE ON FUNCTION app.purge_user(uuid) TO service_role;
-- Orden de eliminación de una cuenta: app.purge_user(id) y después borrar el usuario de
-- auth.users con la API de administración. Lo hace la función de servidor de eliminación (F7).
...
--
-- Supabase concede por defecto todos los privilegios sobre las tablas nuevas de
-- public a anon y authenticated. Algunos (TRUNCATE) no respetan la seguridad a
-- nivel de fila, así que se retiran y se conceden solo los necesarios.
-- ---------------------------------------------------------------------------

REVOKE TRUNCATE, REFERENCES, TRIGGER ON ALL TABLES IN SCHEMA public FROM authenticated, anon;

-- Cuenta: lectura propia y cambio de preferencias; el resto lo gestionan funciones del sistema.
REVOKE ALL ON app_user FROM authenticated;
GRANT SELECT ON app_user TO authenticated;
GRANT UPDATE (locale, time_zone, unit_system) ON app_user TO authenticated;

-- Auditoría y catálogo compartido: solo lectura.
REVOKE ALL ON audit_log, food, food_nutrient FROM authenticated;
GRANT SELECT ON audit_log, food, food_nutrient TO authenticated;
...
CREATE POLICY food_nutrient_read ON food_nutrient FOR SELECT TO authenticated USING (true);

-- Los consentimientos solo los registran funciones del sistema; el usuario los consulta.
REVOKE INSERT, UPDATE, DELETE ON consent FROM authenticated;

-- ---------------------------------------------------------------------------
-- Alta de cuenta y registro completo (RF-01)
--
-- Supabase Auth crea el usuario en auth.users. Un disparador crea su fila en
-- app_user. El registro se completa con fecha de nacimiento (18 años como mínimo)
-- y aceptación de términos, por una de dos vías que usan la misma validación:
--   * registro con correo: la app envía esos datos como metadatos del alta;
--   * Apple o Google: la app llama a public.completar_registro tras el inicio de sesión.
-- ---------------------------------------------------------------------------

-- Versiones vigentes de los textos legales. La app envía la versión que ha mostrado.
CREATE TABLE app.legal_text (
  purpose         text PRIMARY KEY CHECK (purpose IN ('terminos', 'datos_salud')),
  current_version text NOT NULL
);
INSERT INTO app.legal_text (purpose, current_version) VALUES
  ('terminos', '2026-09'),
  ('datos_salud', '2026-09');

CREATE OR REPLACE FUNCTION app.register_initial_data(
  target uuid,
  birth date,
  terms_version text,
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
DECLARE
  vigente_terminos text;
  vigente_salud text;
BEGIN
  IF birth IS NULL THEN
    RAISE EXCEPTION 'FECHA_NACIMIENTO_OBLIGATORIA';
  END IF;
  IF birth < date '1900-01-01' OR birth > current_date THEN
    RAISE EXCEPTION 'FECHA_NACIMIENTO_INVALIDA';
  END IF;
  IF birth > (current_date - interval '18 years')::date THEN
    RAISE EXCEPTION 'EDAD_MINIMA';
  END IF;

  SELECT current_version INTO vigente_terminos FROM app.legal_text WHERE purpose = 'terminos';
  IF terms_version IS DISTINCT FROM vigente_terminos THEN
  ...
  INSERT INTO consent (user_id, purpose, text_version) VALUES (target, 'terminos', terms_version);

  IF coalesce(health_consent, false) THEN
    SELECT current_version INTO vigente_salud FROM app.legal_text WHERE purpose = 'datos_salud';
    IF health_version IS DISTINCT FROM vigente_salud THEN
      RAISE EXCEPTION 'VERSION_DATOS_SALUD_DESACTUALIZADA';
    END IF;
    INSERT INTO consent (user_id, purpose, text_version) VALUES (target, 'datos_salud', health_version);
  END IF;
END $$;

REVOKE ALL ON FUNCTION app.register_initial_data(uuid, date, text, boolean, text) FROM PUBLIC;

-- Disparador sobre auth.users: crea la cuenta de la aplicación.
CREATE OR REPLACE FUNCTION app.handle_new_auth_user() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
DECLARE
  meta jsonb := coalesce(NEW.raw_user_meta_data, '{}'::jsonb);
BEGIN
  INSERT INTO app_user (id, email, email_verified_at)
  VALUES (NEW.id, NEW.email, NEW.email_confirmed_at);

  -- Registro con correo: los datos llegan como metadatos y se validan ya.
  -- Un menor de 18 años hace fallar el alta completa.
  IF meta ? 'fecha_nacimiento' THEN
    PERFORM app.register_initial_data(
      NEW.id,
      (meta ->> 'fecha_nacimiento')::date,
      meta ->> 'version_terminos',
      coalesce((meta ->> 'acepta_datos_salud')::boolean, false),
      meta ->> 'version_datos_salud'
    );
  END IF;
  RETURN NEW;
END $$;
...
-- Mantiene sincronizados el correo y su verificación.
CREATE OR REPLACE FUNCTION app.handle_auth_user_updated() RETURNS trigger
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
BEGIN
  UPDATE app_user
     SET email = NEW.email,
...
-- Vía Apple o Google: la app llama a esta función tras el primer inicio de sesión.
CREATE OR REPLACE FUNCTION public.completar_registro(
  fecha_nacimiento date,
  version_terminos text,
  acepta_datos_salud boolean,
  version_datos_salud text DEFAULT NULL
) RETURNS void
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
DECLARE
  yo uuid := auth.uid();
BEGIN
  ...
  END IF;
  IF EXISTS (SELECT 1 FROM app_user WHERE id = yo AND registration_completed_at IS NOT NULL) THEN
    RETURN;  -- idempotente
  END IF;
  PERFORM app.register_initial_data(yo, fecha_nacimiento, version_terminos, acepta_datos_salud, version_datos_salud);
END $$;

REVOKE ALL ON FUNCTION public.completar_registro(date, text, boolean, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.completar_registro(date, text, boolean, text) TO authenticated;

-- Estado de la cuenta para decidir qué pantalla mostrar tras iniciar sesión.
CREATE OR REPLACE FUNCTION public.estado_cuenta()
RETURNS TABLE (registro_completo boolean, correo_verificado boolean, consentimiento_datos_salud boolean)
LANGUAGE sql STABLE SECURITY INVOKER SET search_path = public, pg_temp AS $$
  SELECT
    u.registration_completed_at IS NOT NULL,
    u.email_verified_at IS NOT NULL,
    EXISTS (
      SELECT 1 FROM consent c
      WHERE c.user_id = u.id AND c.purpose = 'datos_salud' AND c.revoked_at IS NULL
    )
  FROM app_user u
  WHERE u.id = auth.uid()
$$;

REVOKE ALL ON FUNCTION public.estado_cuenta() FROM PUBLIC, anon;
